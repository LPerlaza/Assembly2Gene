...
fromAssembly2gene_forKleb: This program works similarly with the exception that before the prediction of genes uses "mlplasmids" to identify plasmids (https://gitlab.com/sirarredondo/mlplasmids)
fromAssembly2gene_forKleb separates chromosome and plasmids from assembled genomes and automatically find genes of interest. It generates descriptive tables of presence, absence and truncated genes, curated alignments and peptides predictions.		
...
run like: ./fromAssembly2gene.pl -g gene/*fasta -a genome/* -o test														
...
OUTPUTS
	...
	folder _output
		This folder contains all the intermedia files used for the program. Each assembly has two folders one for the chromosome and one for the plasmid. Each folder contains:
						GenomeName.chr.genes.faa:all predicted genes in AA
						GenomeName.chr.genes.fasta: all predicted genes in nt
						GenomeName.chr.genes.gbk: all predicted gnees in gbk format
